...
- `cheni check` : verdict line (`Up to date: X | Minor: Y | ...`) déplacée en haut, avant les sections détails. Skim-readers voient le verdict en premier.
- `cheni diagnose` : findings colorés par sévérité (rouge/jaune/cyan), empty state explicite "✓ No known issues detected".
- `cheni unpin` / `cheni unfreeze` : help discoverable via `--help` (ne nécessite plus de tomber dans le bail "no args").
- `cheni check`, `cheni pin`, `cheni freeze` : empty states pointent vers `cheni init` quand le flake n'est pas configuré.
