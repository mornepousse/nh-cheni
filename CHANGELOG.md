...
- `cheni init` no longer generates an overlay that triggers an `aliases.nix` infinite recursion once a freeze (or a pin) actually fires. The freeze and pin overlay templates (auto-insert + manual instructions) now hardcode `system = "x86_64-linux"` instead of `inherit (prev) system`. Reading `prev.system` while the overlay is being applied forces the fixpoint and loops through `with self;` in nixpkgs `aliases.nix` as soon as the overlay has any non-trivial work to do. Symptom on the user side: `cheni upgrade` aborting with `error: infinite recursion encountered at .../pkgs/top-level/aliases.nix:20: with self;` right after a fresh `cheni freeze`. Existing flakes that were generated by an earlier `cheni init` need the same edit applied manually (or rerun `cheni init` if you want the regenerated snippet).
